...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    # (N, Len_in, n_heads, d_model / n_heads)
    N_, Len_in, n_heads, D_ = value.shape
    ...
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        ...
        value_l_ = value_list[lid_].flatten(2).transpose(1, 2).reshape(N_*n_heads, D_, H_, W_)
        # N_, Len_q, n_heads, n_points, 2 -> N_, n_heads, Len_q, n_points, 2 -> N_*n_heads, Len_q, n_points, 2
        ...
        # N_*n_heads, D_, Len_q, n_points
        sampling_value_l_ = F.grid_sample(value_l_, sampling_grid_l_,
                                          mode='bilinear', padding_mode='zeros', align_corners=False)
        sampling_value_list.append(sampling_value_l_)
    # (N, Len_q, n_heads, n_levels, n_points)
    # (N_, Len_q, n_heads, n_levels, n_points) -> (N_, n_heads, Len_q, n_levels, n_points) -> (N_*n_heads, 1, Len_q, n_levels*n_points)
    attention_weights = attention_weights.transpose(1, 2).reshape(N_*n_heads, 1, Len_q, n_levels*n_points)
    # 将注意力权重和采样特征进行weighted sum
    # (N_*n_heads, D_, Len_q, n_points) -> (N_*n_heads, D_, Len_q, n_level, n_points) -> (N_*n_heads, D_, Len_q, n_level * n_points)
    output = (torch.stack(sampling_value_list, dim=-2).flatten(-2) * attention_weights).sum(-1).view(N_, n_heads*D_, Len_q)
